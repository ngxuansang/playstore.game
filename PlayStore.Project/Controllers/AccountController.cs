...
namespace PlayStore.Project.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    return Json(new { Key = 1, Value = returnUrl });
        ...
        public ActionResult Register()
        {
            ...
            var Cities = db.Database.SqlQuery<Location>("get_city").Select(x => new SelectListItem() { Text = x.LocationName, Value = x.ID.ToString() }).ToArray();
            ViewData["Cities"] = Cities;
            return View();
        }

        // POST: /Account/GetStateListByCity
        [HttpPost]
        [AllowAnonymous]
        public string GetStateListByCity(long cityID)
        {
            DiaGameEntities db = new DiaGameEntities();
            var states = db.Database.SqlQuery<Location>("dbo.get_state_by_city @cityID", new SqlParameter("@cityID", cityID)).ToArray();
        ...
        private interface IEmmurable<T>
        {
        }
